...
function About() {
    ...
    return (
        <div style={style_ip_bg}>
            ...
            {/* Wide screen: text centered with sticky side images */}
            {isWide && (
                <div style={{ position: 'relative', minHeight: '100vh' }}>
                    {/* Left images */}
                    <div style={{ position: 'absolute', left: '7vw', top: 0 }}>
                        <div style={{ position: 'sticky', top: '8rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            <img src={catPic} style={{ ...style_ip_about_image, position: 'relative', top: 'auto', left: 'auto' }} alt="Cat" />
                            <img src={dogPic} style={{ ...style_ip_about_image, position: 'relative', top: 'auto', left: 'auto' }} alt="Dog" />
                        </div>
                    </div>
                    ...
                    {/* Right images */}
                    <div style={{ position: 'absolute', right: '7vw', top: 0 }}>
                        <div style={{ position: 'sticky', top: '8rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            <img src={soccerPic} style={{ ...style_ip_about_image, position: 'relative', top: 'auto', right: 'auto' }} alt="Soccer" />
                            <img src={beachPic} style={{ ...style_ip_about_image, position: 'relative', top: 'auto', right: 'auto' }} alt="Beach" />
                        </div>
                    </div>
                </div>
            )}
            
            {/* Narrow screen: text then images below */}
            {isNarrow && (
                <>
                    <div style={style_ip_about_text}>
                        I'm currently working on two master's degrees in Computer Science and Applied Physics at UNC Charlotte.
