...
function About() {
  ...
  const switchTrack = {
    ...
    background: showInfo ? colors.accent : colors.surface,
    border: `0.5px solid ${showInfo ? colors.accent : colors.border}`,
  ...
  const switchThumb = {
    ...
    left: showInfo ? '22px' : '3px', transition: 'left 0.25s',
      ...
      <div style={style_section}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          ...
          {isMobile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontFamily: fonts.mono, fontSize: '12px', color: colors.muted, letterSpacing: '1px', textTransform: 'uppercase' }}>
                {showInfo ? 'INFO' : 'BIO'}
              </span>
              <div style={switchTrack} onClick={() => setShowInfo(!showInfo)}>
        ...
        {!isMobile && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '28px', alignItems: 'start' }}>
            <BioText />
            <InfoPanel facts={facts} contactLinks={contactLinks} copyEmail={copyEmail} copied={copied} />
          </div>
        )}

        {/* Mobile: toggle between bio and info */}
        {isMobile && !showInfo && <BioText />}
        {isMobile && showInfo && <InfoPanel facts={facts} contactLinks={contactLinks} copyEmail={copyEmail} copied={copied} />}
